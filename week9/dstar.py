...
class Dstar:
        ...
        if k_old < x.h:
            for y in self.map.get_neighbors(x):
                if y.h <= k_old and x.h > y.h + x.cost(y):
                    x.parent = y
                    x.h = y.h + x.cost(y)  # x의 cost를 또다른 최소 비용을 갖는 방향으로 수정 
        if k_old != x.h:
            for y in self.map.get_neighbors(x):  # x의 cost가 변화하면서 영향을 받은 이웃들을 재검사 (현재 state의 cost보다 작은 cost를 가지고 있는 경우 검사)
                if y.t == "new" or y.parent == x and y.h != x.h + x.cost(y): # 방문하지 않았던 cell이나 원래 x를 거쳐서 목적지로 도달하는 경로인 cell들
                    y.parent = x
                    self.insert(y, x.h + x.cost(y))
                else:
                    if y.parent != x and y.h > x.h + x.cost(y): # x보다 앞선 step에서 계산된 cell들이 x를 거쳐가면 더 cost가 낮아지게 되는 경우
                        y.parent = x
                        self.insert(y, x.h + x.cost(y))
                    else:
                        if y.parent != x and x.h > y.h + x.cost(y) and y.t == "close" and y.h > k_old: # 앞에서 x의 parent에 할당된 최소 비용의 cell은 아니지만, 변화한 환경에서 x에서 가면 이득이 되는 cell들
                            self.insert(y, y.h) # 코스트가 x.h보다 작으므로, 다시한번 검사
    ...
    def modify(self, state):
        ...
        while True:
            ...
            if k_min >= state.h:  # open_list의 최소 k가 현재 state의 cost보다 커지면 더 이상 검사를 할 필요가 없음
                break
